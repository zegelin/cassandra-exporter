...
public abstract class SimpleCollector<T> implements MBeanGroupMetricFamilyCollector {
    protected final String name, help;

    protected final Constructor<T> constructor;

    // labels -> MBeans
    protected final Map<Labels, T> metrics;

    @FunctionalInterface
    public interface Constructor<T> {
        MBeanGroupMetricFamilyCollector instantiate(final String name, final String help, final Map<Labels, T> metrics);
    }
    ...
    public MBeanGroupMetricFamilyCollector merge(final MBeanGroupMetricFamilyCollector other) {
        ...
        final HashMap<Labels, T> metrics = new HashMap<>(this.metrics);
        for (final Map.Entry<Labels, T> entry : ((SimpleCollector<T>) other).metrics.entrySet()) {
    ...
    protected <MetricT> Set<MetricT> transformMetrics(final Maps.EntryTransformer<Labels, T, MetricT> entryTransformer) {
