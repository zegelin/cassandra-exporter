...
import com.zegelin.prometheus.cassandra.MBeanMetricFamilyCollectorFactory;
...
public class FailureDetectorMBeanMetricFamilyCollector implements MBeanMetricFamilyCollector {
    ...
    private static final Map<String, String> ENDPOINT_STATE_DOWN_LABELS = ImmutableMap.of("state", "down");
    private static final Map<String, String> ENDPOINT_STATE_UP_LABELS = ImmutableMap.of("state", "up");

    public static final MBeanMetricFamilyCollectorFactory FACTORY = mBean -> {
    ...
    public Stream<MetricFamily<?>> collect() {
        ...
        try {
            ...
            final Set<NumericMetric> phiMetrics = endpointPhiValues.stream().map(d -> new NumericMetric(
                    ImmutableMap.of("endpoint", ((String) d.get("Endpoint")).substring(1)),
                    (Double) d.get("PHI"))
